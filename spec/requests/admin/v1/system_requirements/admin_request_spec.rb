...
RSpec.describe "Admin::V1::SystemRequirements as :admin", type: :request do
  ...
  context "POST /system_requirements" do
    ...
    context "with valid params" do
      ...
      it "add new system_requirement" do
        expect do
          post url, headers: auth_header(user), params: system_requirement_params
        end.to change(SystemRequirement, :count).by(1)
      end

      it "returns last added system_requirements" do
        post url, headers: auth_header(user), params: system_requirement_params
        expected_system_requirement = SystemRequirement.last.as_json(only: %i(id name os storage processor memory video_board))
  ...
  context "PATCH /system_requirements" do
    ...
    context "with valid params" do
      let!(:new_name) { "My new system_requirement #{system_requirement.id}" }
      ...
      it "returns updated system_requirement" do
        patch url, headers: auth_header(user), params: system_requirement_params
        system_requirement.reload
        expected_system_requirement = system_requirement.as_json(only: %i(id name os storage processor memory video_board))
        expect(body_json["system_requirement"]).to eq expected_system_requirement
      end
